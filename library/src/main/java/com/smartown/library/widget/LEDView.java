...
public abstract class LEDView extends View {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        LogTool.i("onMeasure", "onMeasure");
        rowHeight = (getRowNum() + 2) * (pointWidth + intervalWidth);
        int rowWidth;
        int rowSize = 1;
        maxColumnSize = (Tool.getScreenWidth(getContext()) / (pointWidth + intervalWidth) - 1) / (getColumnNum() + 1);
        int length = number.length();
        if (length > maxColumnSize) {
            rowWidth = Tool.getScreenWidth(getContext());
